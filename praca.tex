...
% Wynik naiwnego modelu Bayesa\cite{bayes} widoczny na wykresie \ref{fig:plot_gauss} był bardzo podobny do linii diagonalnej, co oznacza, że zwykłe losowanie przyniosło by w tym przypadku podobne efekty. Klasyfikator ten jest modelem probalistycznym, który bazuje na teorii Bayesa opisanej wzorem: 
% $
% P(A|B) = \frac{P(B|A)*P(A)}{P(B)} 
% $
% \begin{figure}[h!]
%     \centering
%     \includegraphics[width=\textwidth]{images/gaus.png}
%     \caption{ROC AUC dla funkcji Gaussa.\protect\footnotemark}
%     \label{fig:plot_gauss}
% \end{figure}
% Drzewa decyzyjne pokazane na rysunku \ref{fig:plot_tree} i sieć neuronowa na rysunku \ref{fig:plot_mlp} wykazały pole pod krzywą mniejsze niż 0.5, co oznacza, że klasyfikatory te często prognozowały klasę przeciwną.
...
Ostatnim modelem poddanym eksperymentowi jest MLP widoczny na rysunku \ref{fig:plot_mlp}. Model ten uzyskał największą średnią ROC AUC, ale jego interpretacja jest dużo trudniejsza niż w przypadku regresji logistycznej. Sieć neuronowa na ogół składa się z jednej warstwy do wprowadzania danych (z ang. \ang{input layer}), jednej warstwy do wyprowadzania danych (z ang. \ang{output layer}) oraz jednej lub więcej warstw ukrytych (z ang. \ang{hidden layer}).

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{images/mlp.png}
    \caption{ROC AUC dla MLP.\protect\footnotemark}
    \label{fig:plot_mlp}
\end{figure}

W warstwie ukrytej znajdują się tzw. neurony, które otrzymują dane z warstw poprzednich pomnożone przez pewne wagi ustalane podczas procesu tworzenia sieci, czyli modelu. Neurony po otrzymaniu danych używają z góry zdefiniowanych funkcji aktywacyjnych. Przekształcają one informacje wejściowe i przesyłają je dalej. Cały proces składa się z wielu iteracji, gdzie za jedną iterację uważa się przejście danych przez wszystkie warstwy ukryte aż do warstwy wyjściowej (z ang. \ang{forward propagation}) i etap "uczenia" korygujący wagi pomiędzy warstwami, podczas którego zmiany propagują się od strony wyjściowej do wejściowej (z ang. \ang{back propagation}). Dla sieci użytej w projekcie zostały użyte pewne empirycznie wybrane hiperparametry. Maksymalna ilość iteracji została ustawiona na 1000. Jako funkcje aktywacyjną wybrano ``relu`` opisany wzorem $ max(0, x) $. Rozmiar sieci neuronowej to jedna warstwa, w której zawiera się 25 neuronów.   
...
\item[AWS] Zbiór gotowych do użycia serwisów udostępnionych przez przedsiębiorstwo Amazon (ang. \ang{Amazon Web Services})

\item[EC2] Serwis WWW dostarczający skalowalną moc obliczeniową w chmurze obliczeniowej (ang. \ang{Amazon Elastic Compute Cloud }) 

\item[API] Interfejs programowania aplikacji, interfejs programistyczny aplikacji, API (ang. \ang{application programming interface}) 
